...
def init():
	...
	if not phone:
		...
		if not list_user:
			phone = input(f"{lg}[*]{lx} Country Code + Phone Number (ex:+62812..): ")
			while not "+" in phone:
				print(f'{lr}[*]{lx} Please use country code in your number ex: +62, +61')
				phone = input(f"{lg}[*]{lx} Country Code + Phone Number (ex:+62812..): ")
		else:
...
def get_group_members(client):
	...
	members = []
	all_members = client.get_participants(sltd,aggressive=False)
	...
	if os.listdir("results"):
		...
		for user in slxtd:
			try:
				...
				waitime = re.search(r'(\d+)',e).group(1)
				print(f"    {lr}!{lx} FloodWaitError detected, and we must wait for {waitime} seconds..")
				loading(int(waitime))
				continue
			except Exception as e:
